...
//		JLabel lblNewLabel = new JLabel("上传Java项目");
		JLabel lblNewLabel = new JLabel("Upload Java projects");
...
//		JLabel lblNewLabel_1 = new JLabel("请选择.zip压缩包格式的Java项目源代码文件");
		JLabel lblNewLabel_1 = new JLabel("Please select the source code of Java projects in.zip compressed package format.");
...
//		JLabel rootLabel = new JLabel("项目地址：");
		JLabel rootLabel = new JLabel("Project address：");
...
//		JButton chooseButton = new JButton("选择文件");
		JButton chooseButton = new JButton("Select files");
...
//			        	JOptionPane.showMessageDialog(null, "请选择.zip压缩文件", "文件类型不支持", JOptionPane.WARNING_MESSAGE);
			        	JOptionPane.showMessageDialog(null, "Please select the.zip compressed file.", "The file type is not supported.", JOptionPane.WARNING_MESSAGE);
			        }else {
			        	rootTextField.setText(file.getAbsoluteFile().toString());
			        }
		        }else if(result == JFileChooser.CANCEL_OPTION) {
//		        	rootTextField.setText("未选择文件");
		        	rootTextField.setText("No file selected");
		        }
			}
		});
...
//								JOptionPane.showMessageDialog(null, "解析成功", "提示", JOptionPane.INFORMATION_MESSAGE);
								JOptionPane.showMessageDialog(null, "Parsing succeeded.", "Prompt", JOptionPane.INFORMATION_MESSAGE);
							}catch(Exception e) {
								uploadButton.setEnabled(true);
								progressBar.dispose();
								e.printStackTrace();
//								JOptionPane.showMessageDialog(null, "解析失败，请重试！", "错误 ", JOptionPane.WARNING_MESSAGE);
								JOptionPane.showMessageDialog(null, "Parsing failed. Please try again!", "error ", JOptionPane.WARNING_MESSAGE);
							}
						}
					}).start();
				}else {
//					JOptionPane.showMessageDialog(null, "请输入正确的文件路径和文件格式", "错误", JOptionPane.WARNING_MESSAGE);
					JOptionPane.showMessageDialog(null, "Please enter the correct file path and file format.", "error", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
